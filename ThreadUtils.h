...
void ThreadPool::run_pending_task()
{
	...
	bool res;
	int i = 0;
	for (; i < m_thread_count; ++i)	//ÏÀÍÈ×ÅÑÊÈ ÏÛÒÀÅÌÑß ÏÎËÓ×ÈÒÜ ÕÎÒÜ ÊÀÊÓÞ-ÒÎ ÇÀÄÀ×Ó
	{
		res = m_queues[i % m_thread_count].fast_pop(task_to_do);
		if (res) break;
	}

	if (!task_to_do.valid())	//ß âîîáùå íå óâåðåí, íàñêîëüêî ïðàâèëüíî òàê äåëàòü. Íå ìîãó ïîêà äî êîíöà ïðîäóìàòü âñå âàðèàíòû, êîãäà ýòî ìîæíò âûçâàòü ïðîáëåìû
	{							//Ïîäñìîòðåë ýòî ðåøåíèå ÿ ó Andrew Williams â êíèãå ïðî ìíîãîïîòî÷íîñòü, 2ÿ ðåäàêöèÿ (ãäå Ñ++ 17ãî ñòàíäàðòà)
